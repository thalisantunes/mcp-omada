...
async def test_list_devices_legacy_is_normalized(
    ...
    rows = result["result"]
    assert len(rows) == 3
    ap = next(r for r in rows if r["mac"] == "50-D4-F7-10-20-30")
...
async def test_get_device_detail_happy_path_includes_raw(
    ...
    _content, result = await mcp.call_tool(
        "get_device_detail", {"mac": "50:d4:f7:10:20:30", "site_id": fake_controller.site_id}
    )
    assert result["mac"] == "50-D4-F7-10-20-30"
...
async def test_get_wifi_summary_filters_by_mac(
    ...
    mcp = build_server(settings=settings_legacy, client_factory=_factory(transport))
    _content, result = await mcp.call_tool(
        "get_wifi_summary", {"site_id": fake_controller.site_id, "mac": "50-D4-F7-10-20-31"}
    )
    rows = result["result"]
    assert len(rows) == 1
...
async def test_set_radio_channel_preview_then_confirm(
    ...
    _content, preview = await mcp.call_tool(
        "set_radio_channel",
        {"mac": "50:d4:f7:10:20:30", "band": "2g", "channel": 6, "site_id": fake_controller.site_id},
    )
    ...
    _content, applied = await mcp.call_tool(
        "set_radio_channel",
        {"mac": "50:d4:f7:10:20:30", "band": "2g", "channel": 6, "confirm": True, "site_id": fake_controller.site_id},
    )
...
async def test_set_radio_channel_invalid_band_rejected_before_touching_device(settings_legacy_write_enabled: Settings):
    ...
    with pytest.raises(ToolError) as exc_info:
        await mcp.call_tool(
            "set_radio_channel", {"mac": "50-D4-F7-10-20-30", "band": "6g", "channel": 6, "confirm": True}
        )
